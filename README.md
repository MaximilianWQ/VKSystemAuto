...
Сборка идёт по `Dockerfile`: отдельная стадия собирает фронтенд, рантайм
ставит только боевые зависимости через `uv sync --no-dev`.

### Два момента, о которых надо знать

**Сервис всё ещё подключён к репозиторию старого бота.** В настройках `worker`
источником указан `novikovLDN/STAGEv`, ветка `claude/vk-bot-setup-Z72l7`, а
командой запуска на уровне сервиса — `python bot.py`. Наш `railway.json`
перекрывает команду при каждом деплое, поэтому сейчас всё работает. Но пуш в тот
старый репозиторий может запустить деплой старого бота поверх нового. Правильно
отвязать источник или переключить его на `MaximilianWQ/VKSystemAuto`.

**`railway.json` устарел** и перестанет работать после 2026-12-01. Автомиграция
в `.railway/railway.ts` не подходит как есть: инфраструктура-как-код считает
необъявленные переменные лишними, и сухой прогон показал удаление всех десяти
переменных окружения, включая токен ВК и подключение к базе. Объявить их в
репозитории нельзя — это секреты. Переносить нужно вместе с переводом переменных
на shared variables или с явным исключением их из-под управления IaC.
